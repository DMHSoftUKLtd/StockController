...
namespace StockController
{
    public partial class ShopSaleForm : Form
    {
        ...
        private void ShopSaleForm_Load(object sender, EventArgs e)
        {
            LoadShopsIntoForm();
            LoadStockIntoForm();
            DtpTransaction.Value = Utility.GetSundayDate(DtpTransaction.Value, 1);
            if (FormMode == "New")
            {
                cmdOK.Text = "Save";
        ...
        {
            ShopSale saleHead = new ShopSale();
            ShopSale saleLine = new ShopSale();
            Log logs = new Log();
            decimal toConvert;
            ...
            decimal.TryParse(txtTotal.Text, NumberStyles.AllowCurrencySymbol | NumberStyles.AllowDecimalPoint | NumberStyles.AllowThousands, CultureInfo.CurrentCulture, out toConvert);
            saleHead.Value = toConvert;
            saleHead.Qty = Convert.ToInt32(txtTotalGarments.Text.TrimEnd());
            decimal.TryParse(txtVAT.Text, NumberStyles.AllowCurrencySymbol | NumberStyles.AllowDecimalPoint | NumberStyles.AllowThousands, CultureInfo.CurrentCulture, out toConvert);
        ...
        private void TxtShopRef_Leave(object sender, EventArgs e)
        {
            ...
            lblShopName.Text = Shops.GetShopNameFromDB();
            LoadSalesGrid();
        }
        ...
        private void DtpTransaction_ValueChanged(object sender, EventArgs e)
        {
            DtpTransaction.Value = Utility.GetSundayDate(DtpTransaction.Value, 1);
        }
